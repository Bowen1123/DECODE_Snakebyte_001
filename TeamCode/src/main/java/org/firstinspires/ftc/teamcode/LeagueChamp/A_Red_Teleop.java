...
public class A_Red_Teleop extends LinearOpMode {
    ...
    ///  Preset Values
    private final double INT_shooter_power = 0.4, INT_transfer_power = 0.95, INT_intake_power = .85, INT_dt_max_power = 0.7;
    ...
    /// Positions
    private Pose2d robot_pose = new Pose2d(-60, 60, Math.toRadians(135));
    ...
    // Turret IMU
    public static double turretOffsetRad = Math.toRadians(0);
    ...
    private boolean d2control = false;
    ...
    public void runOpMode() throws InterruptedException {
        ...
        if (StorePose.init != null){
            robot_pose = StorePose.init;
        }
        ...
        while (opModeIsActive()){
            ...
            if (gamepad1.right_trigger > 0.2 /*|| gamepad2.right_trigger > 0.2*/) {
                ...
                transfer.setPower(transfer_power * .45);
...
//            }
            ...
            if (!d2control) {
                ...
                if (gamepad2.yWasPressed()){
                    shooterRamp.setPosition(Math.max(Math.min(shooterRamp.getPosition() + 0.1, .7), .3));
